...
// Polymarket kadang return 46 artinya 0.46
...
export async function getOrderBook(
  ...
  try {
    ...
    console.error(
      `[Depth] orderbook fetch failed: ${err}`
    );
    return null;
  }
}
...
export async function isLiquidEnough(
 ...
 if(
   ...
   console.log(
    `[Depth] insufficient liquidity ${filled.toFixed(1)}/${requiredShares.toFixed(1)}`
   );
 ...
 console.log(
  `[Depth] filled ${filled.toFixed(1)} shares | slippage ${slippage.toFixed(2)}%`
 );
...
export async function getMarketVolume(
 ...
 try{
  ...
  return {
   volume24h:
    res.data?.volume_24h || 0,

   volume7d:
    res.data?.volume_7d || 0
  };
